...
/**
 * This class implements the SoPeCo extension for the different breakdown exploration strategies.
 * 
 * @author Dennis Westermann, Rouven Krebs
 */
public class BreakdownExplorationController extends AbstractSoPeCoExtensionArtifact implements
		IMeasurementSeriesAwareable, IExplorationStrategy {
	...
	private void createBreakdownImplementation(List<IParameterVariation> parameterVariations, ExperimentSeriesRun expSeriesRun){
		...
		if (strategyConfig.getName().equalsIgnoreCase(BreakdownConfiguration.EQUIDISTANT_BREAKDOWN)) {
			breakdownImpl = new EquidistantBreakdownImpl(strategyConfig, environmentCachedAcess,
					parameterDefinitions, stopController, provider);
		} else if (strategyConfig.getName().equalsIgnoreCase(BreakdownConfiguration.RANDOM_BREAKDOWN)) {
			breakdownImpl = new RandomBreakdownImpl(strategyConfig, environmentCachedAcess, 
					parameterDefinitions, stopController, provider);
		} else if (strategyConfig.getName().equalsIgnoreCase(BreakdownConfiguration.ADAPTIVE_EQUIDISTANT_BREAKDOWN)) {
			breakdownImpl = new AdaptiveEquidistantBreakdownImpl(strategyConfig, environmentCachedAcess,
					parameterDefinitions, stopController, provider);
		} else if (strategyConfig.getName().equalsIgnoreCase(BreakdownConfiguration.ADAPTIVE_RANDOM_BREAKDOWN)) {
			breakdownImpl = new AdaptiveRandomBreakdownImpl(strategyConfig, environmentCachedAcess,
					parameterDefinitions, stopController, provider);
		} else {
	...
	private AlgorithmsEnvironment createAlgorithmEnvironment(
			List<IParameterVariation> parameterVariations, ExperimentSeriesRun expSeriesRun) {
	
		return new AlgorithmsEnvironment(experimentController, 
				persistenceProvider, getAnalyzer(), strategyConfig.getAnalysisConfigurations().get(0), 
				parameterVariations, BreakdownConfiguration.getMainPerformanceIndicator(strategyConfig, expSeriesRun), 
				expSeriesRun);
	...
	private List<ParameterDefinition> getParameterDefinitions(List<IParameterVariation> parameterVariations) {
		List<ParameterDefinition> allParameters = new ArrayList<ParameterDefinition>(
				parameterVariations.size());
	...
	private IPredictionFunctionStrategy getAnalyzer() {
		...
		if (strategyConfig.getAnalysisConfigurations().isEmpty()) {
			  // try to get any analysis TODO: change from any to a specific from our std.analyses?
			  Extensions<IPredictionFunctionStrategyExtension> availableExtensions = 
					  extensionRegistry.getExtensions(IPredictionFunctionStrategyExtension.class);
			...
			return extensionRegistry.getExtensionArtifact(IPredictionFunctionStrategyExtension.class,
					strategyConfig.getAnalysisConfigurations().get(0).getName());
		}
